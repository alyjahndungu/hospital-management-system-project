...
import com.lanmedics.lanmedics.modal.*;
...
public class DatabaseHelper {
	...
	public void createUser(HttpServletRequest request) {
		try {
			...
			ps = connection.prepareStatement(
					"insert into users(firstName,lastName,email,password,dob,token,personType,gender,address,contactNumber) values(?,?,?,?,?,?,?,?,?,?)");
			...
			ps.executeUpdate();
			System.out.println("Execution Data" + ps);
			ts = connection.prepareStatement("select userId from users where email=?");
			ts.setString(1, request.getParameter("email"));
	...
	public int[] getPersonType(String email, String password) {
		int result[] = { -1, 0 };
	...
	public List<Users> list() throws SQLException{
		List<Users> doctorsList = new ArrayList<>();
		PreparedStatement ps = connection.prepareStatement("select * from users where personType = 2");
		ResultSet rs = ps.executeQuery();
		while (rs.next()){
			int id = rs.getInt("userId");
			String firstName = rs.getString("firstName");
			String lastName = rs.getString("lastName");
			Users appointment = new Users(id, firstName, lastName);
	...
	public Users getUser(int id) {
		try {
			...
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				return new Users(rs.getInt("userId"), rs.getString("firstName"), rs.getString("lastName"),
						rs.getString("email"), rs.getLong("dob"), rs.getInt("personType"), rs.getString("gender"),
	...
	public ArrayList<Users> getUser() {
		try {
			...
			while (rs.next()) {
				Users user = new Users(rs.getInt("userId"), rs.getString("firstName"), rs.getString("lastName"),
						rs.getString("email"), rs.getLong("dob"), rs.getInt("personType"), rs.getString("gender"),
						rs.getString("address"), rs.getString("contactNumber"));
				users.add(user);
			}
	...
	public int updateUser(int patientId, HttpServletRequest request) {
		try {
			// update user
			if (!request.getParameter("password").isEmpty()) {
				PreparedStatement ps = connection.prepareStatement(
						"update users set password=?, dob=?, address=?, contactNumber=? where userId=?");
				...
				PreparedStatement ps = connection
						.prepareStatement("update users set dob=?, address=?, contactNumber=? where userId=?");
	...
	public Patient getPatient(int id) {
		try {
			...
			if (rs.next()) {
				...
				if (rsPatient.next()) {
					return new Patient(rs.getInt("userId"), rs.getString("firstName"), rs.getString("lastName"),
							rs.getString("email"), rs.getLong("dob"), rs.getInt("personType"),
	...
	public Doctor getDoctor(int id) {
		try {
			...
			if (rs.next()) {
				...
				if (rsDoctor.next()) {
					return new Doctor(rs.getInt("userId"), rs.getString("firstName"), rs.getString("lastName"),
							rs.getString("email"), rs.getLong("dob"), rs.getInt("personType"),
							rs.getString("gender"), rs.getString("address"), rs.getString("contactNumber"),
	...
	public Staff getStaff(int id) {
		try {
			...
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				return new Staff(rs.getInt("userId"), rs.getString("firstName"), rs.getString("lastName"),
						rs.getString("email"), rs.getLong("dob"), rs.getInt("personType"), rs.getString("gender"),
						rs.getString("address"), rs.getString("contactNumber"));
			}
	...
	public int createAppointment(Appointment appointment) {
		...
		try {
			...
			PreparedStatement ps = connection.prepareStatement(
					"insert into appointment (patientId, dateCreated, symptoms, disease, preferredDate, "
							+ "title) values(?,?,?,?,?,?)");
			ps.setInt(1, appointment.getPatient().getId());
			ps.setLong(2, System.currentTimeMillis());
			ps.setString(3, appointment.getSymptoms());
	...
	public ArrayList<ArrayList<Appointment>> getAppointments(int userId, String mactingColumn) {
		...
		try {
			...
			if(mactingColumn!=null)
			{
				arrayList.add(upcomingAppointments);
	...
	public Appointment getDetailedAppointment(int appointmentId) {
		try {
			...
			if (rs.next()) {
				...
				Patient patient = getPatient(patientId);
				Appointment appointment = new Appointment(id, doctor, rs.getString("title"), rs.getLong("dateCreated"),
						rs.getString("symptoms"), rs.getString("disease"), patient, rs.getLong("allocatedDate"), rs.getLong("preferredDate"), rs.getInt("isClosed"));
		...
		try {
			...
			while (rs.next()) {
				...
				Appointment appointment = new Appointment(rs.getInt("appointmentId"),doctor, patient, rs.getInt("isClosed"), 
						...
						rs.getString("symptoms"), rs.getString("disease"), rs.getString("title"));
	...
	public int getAppointmentCount(int userId, String matchingColumn){
		PreparedStatement ps;
		try {
	...
	public int getPatientCount(int userId){
		PreparedStatement ps;
		try {
	...
	public ArrayList<Doctor> getDoctors() {
		try {
			// get user from database
			PreparedStatement ps = connection.prepareStatement("select userId from users where personType=2");
			ResultSet rs = ps.executeQuery();
			ArrayList<Doctor> arrayList = new ArrayList<>();
			while (rs.next()) {
				// get doctor from database
				arrayList.add(getDoctor(rs.getInt("userId")));
			}
			return arrayList;
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public ArrayList<Staff> getStaff() {
		try {
			...
			PreparedStatement ps = connection.prepareStatement("select userId from users where personType<>2 AND personType<>1");
			ResultSet rs = ps.executeQuery();
			ArrayList<Staff> arrayList = new ArrayList<>();
			while (rs.next()) {
				// get doctor from database
				arrayList.add(getStaff(rs.getInt("userId")));
			}
			return arrayList;
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public ArrayList<Patient> getPatients() {
		try {
			// get user from database
			PreparedStatement ps = connection.prepareStatement("select userId from users where personType=1");
			ResultSet rs = ps.executeQuery();
			ArrayList<Patient> arrayList = new ArrayList<>();
			while (rs.next()) {
				// get doctor from database
				arrayList.add(getPatient(rs.getInt("userId")));
			}
			return arrayList;
		} catch (SQLException e) {
			e.printStackTrace();
